...
int main(int argc,char** argv){
  ...
  if (tab_fd[0].revents==POLLIN){
    ...
    if(strncmp(message,"/send",5) == 0){
      ...
      if(strcmp(answer, "yes\n") == 0){
        ...
        sprintf(where,"/home/berger/T2/ProgR/src/jalon01/toreceive/%s",file);
  ...
  if (tab_fd[1].revents==POLLIN){
    ...
    if (strcmp(text,"/quit\n") == 0){
      ...
      int length3 = strlen(file);
      char *short_chaine =malloc(MAX_LENGTH_MESSAGE);
      strncpy(short_chaine,file,length3-1);


      char *where = malloc(MAX_LENGTH_MESSAGE);
      sprintf(where,"/home/berger/T2/ProgR/src/jalon01/tosend/%s",short_chaine);
